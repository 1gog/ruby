...
RUN yum install epel-release -y && \
    yum install -y openssl-devel postgresql-libs nodejs postgresql-devel postgresql readline-devel zlib-devel gcc bzip2 vim git make && yum clean all -y 
...
RUN eval "$(rbenv init -)" && rbenv local ${RUBY_VERSION} && rbenv global ${RUBY_VERSION} && gem install rails 5.2.2
